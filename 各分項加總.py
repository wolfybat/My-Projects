...
# Create button
DoButton = Button(root, text="2. 輸出各項加總", command=GetSum, height=5, width=15, font=myFont, bg=buttonBG, activebackground=buttonPressBG)
browseButton = Button(root, text="瀏覽檔案", command=Browse, font=myFont, bg=buttonBG, activebackground=buttonPressBG)
enterButton = Button(root, text="1. 確定", command=Enter, font=myFont, bg=buttonBG, activebackground=buttonPressBG)
DoButton.pack(pady=30)

# Create input box
filenameLabel = Label(root, text="檔案名稱 :  ", bg=bg, font=entryFont)
fileName = Label(root, text=" ", bg=bg, font=entryFont)
firstColLabel = Label(root, text="第一欄位置 :  ", bg=bg, font=entryFont)
secColLabel = Label(root, text="第二欄位置 :  ", bg=bg, font=entryFont)
firstCol = Entry(root, font=entryFont)
secCol = Entry(root, font=entryFont)
saveColFirstLabel = Label(root, text="存去的第一欄位置 :  ", bg=bg, font=entryFont)
saveColFirst = Entry(root, font=entryFont)
saveColSecLabel = Label(root, text="存去的第二欄位置 :  ", bg=bg, font=entryFont)
